...
_formatter_mod = _load_module_direct("nemo.collections.common.prompts.formatter",
                                     _os.path.join(_PROMPTS_DIR, "formatter.py"))
_qwen_mod = _load_module_direct("nemo.collections.common.prompts.qwen",
                                 _os.path.join(_PROMPTS_DIR, "qwen.py"))
_nano_mod = _load_module_direct("nemo.collections.common.prompts.nemotron_nano_v3",
                                 _os.path.join(_PROMPTS_DIR, "nemotron_nano_v3.py"))
...
class TestNemotronNanoV3ChatTemplate:
    ...
    def test_full_inference_format(self, template):
        ...
        result = _render_jinja(template, [{"role": "user", "content": [
            {"type": "text", "text": "Transcribe:"},
            {"type": "audio", "audio": {}}
        ]}])
        expected = (
...
class TestToJinjaBaseClass:
    ...
    def test_custom_audio_token(self):
        ...
        result = _render_jinja(tmpl, [{"role": "user", "content": [
            {"type": "text", "text": "Hi"},
